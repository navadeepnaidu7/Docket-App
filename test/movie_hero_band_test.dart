...
void main() {
  ...
  Future<void> pumpFace(
    WidgetTester tester,
    MoviePass pass, {
    MovieTicketDensity density = MovieTicketDensity.glance,
  }) async {
        ...
        home: Scaffold(
          body: SizedBox(
            ...
            child: SingleChildScrollView(
              child: MovieTicketFace(pass: pass, density: density),
            ),
          ),
        ),
      ),
  ...
  group('movie hero band', () {
    for (final MoviePassBrand brand in MoviePassBrand.values) {
      testWidgets(
        'does not render brand chip or status pill for ${brand.name}',
        (WidgetTester tester) async {
          ...
          expect(
            find.text(MovieBrandStyle.forPass(pass).chipLabel),
            findsNothing,
    ...
    testWidgets('does not show expired pill for an expired pass', (
      WidgetTester tester,
    ) async {
      await pumpFace(
        tester,
    ...
    testWidgets('requests no image when there is no poster URL', (
      WidgetTester tester,
    ) async {
    ...
    testWidgets('prefers a bundled asset over the network URL', (
      WidgetTester tester,
    ) async {
      await pumpFace(
        tester,
    ...
    // Design probe: glance prefers the transparent logo when both URLs exist.
    testWidgets('glance uses the logo URL when both logo and poster are set', (
      WidgetTester tester,
    ) async {
      const String logo =
          'https://api.docket.app/img/poster/w500/eYvF1LhPKuoBxOAmWjFTAK7EPWl.png';
      const String poster =
          'https://api.docket.app/img/poster/w500/heM4XKC0jA8fTSNe8F7oUkcJV7Z.jpg';
      await pumpFace(
        tester,
        buildPass(
          brand: MoviePassBrand.bookMyShow,
      ...
      expect(image.imageUrl, logo);
      expect(image.fit, BoxFit.contain);
    });

    testWidgets('detail uses the poster even when a logo is present', (
      WidgetTester tester,
    ) async {
      const String logo =
          'https://api.docket.app/img/poster/w500/eYvF1LhPKuoBxOAmWjFTAK7EPWl.png';
