...
- **Ticket code extraction** (`features/tickets/application/ticket_code_scanner.dart`): the real gate code is decoded off the uploaded ticket **on device** with ML Kit before the extract upload — images directly, PDFs rasterised page by page through `pdfx` — and sent as extra fields on `POST /tickets/extract`, which the server validates and stores in `metadata`. Costs nothing: no tokens, no server CPU. All symbologies, not just QR: `PassCodeView` renders QR through `qr_flutter` and everything else through `barcode_widget`, because a Code 128 strip re-rendered as a QR scans nowhere. Selection between multiple symbols in one screenshot is a pure, tested function — marketing and app-store payloads dropped, then largest wins. **Every fake code is gone**: `TicketQrPainter`, the decorative `PassCodeBlock` grid and the train sheet's Material glyph are deleted, and a pass with no code draws none. Design decisions in `docs/features/ticket-code-extraction.md`.
